...
public class XYShapeQueryIT extends GeospatialRestTestCase {
    ...
    public void testNullShape() throws Exception {
        queryTestHelper.queryTestUsingNullShape(indexName, xyShapeFieldName, XYShapeFieldMapper.CONTENT_TYPE);
    ...
    public void testIndexPointsFilterRectangleWithDefaultRelation() throws Exception {
        ...
        final String firstDocumentID = queryTestHelper.indexDocumentUsingWKT(indexName, xyShapeFieldName, "POINT(-30 -30)");
        queryTestHelper.indexDocumentUsingGeoJSON(indexName, xyShapeFieldName, new Point(-45, -50));
    ...
    public void testIndexPointsFilterRectangleWithIntersectsRelation() throws Exception {
        createIndex(indexName, Settings.EMPTY, Map.of(xyShapeFieldName, XYShapeFieldMapper.CONTENT_TYPE));
        final String firstDocumentID = queryTestHelper.indexDocumentUsingWKT(indexName, xyShapeFieldName, "POINT(-30 -30)");
        queryTestHelper.indexDocumentUsingWKT(indexName, xyShapeFieldName, "POINT(-45 -50)");

        Rectangle rectangle = new Rectangle(-45, 45, 45, -45);
        final SearchResponse searchResponse = queryTestHelper.searchUsingIntersectsRelation(indexName, xyShapeFieldName, rectangle);
        assertSearchResponse(searchResponse);
        assertHitCount(searchResponse, 1);
        MatcherAssert.assertThat(searchResponse.getHits().getAt(0).getId(), equalTo(firstDocumentID));
    ...
    public void testIndexPointsCircle() throws Exception {
        ...
        queryTestHelper.indexDocumentUsingWKT(indexName, xyShapeFieldName, "POINT(-131 -30)");
        final String secondDocumentID = queryTestHelper.indexDocumentUsingWKT(indexName, xyShapeFieldName, "POINT(-45 -50)");

        Circle circle = new Circle(-30, -30, 100);

        final SearchResponse searchResponse = queryTestHelper.searchUsingIntersectsRelation(indexName, xyShapeFieldName, circle);
        assertSearchResponse(searchResponse);
        assertHitCount(searchResponse, 1);
    ...
    public void testIndexPointsPolygon() throws Exception {
        ...
        final SearchResponse searchResponse = queryTestHelper.searchUsingIntersectsRelation(indexName, xyShapeFieldName, polygon);
        assertSearchResponse(searchResponse);
        assertHitCount(searchResponse, 1);
        MatcherAssert.assertThat(searchResponse.getHits().getAt(0).getId(), equalTo(firstDocumentID));
    ...
    public void testIndexPointsMultiPolygon() throws Exception {
        ...
        final String firstDocumentID = queryTestHelper.indexDocumentUsingWKT(indexName, xyShapeFieldName, "POINT(-30 -30)");
        queryTestHelper.indexDocumentUsingWKT(indexName, xyShapeFieldName, "POINT(-40 -40)");
        final String thirdDocumentId = queryTestHelper.indexDocumentUsingGeoJSON(indexName, xyShapeFieldName, new Point(-50, -50));
        ...
        final SearchResponse searchResponse = queryTestHelper.searchUsingIntersectsRelation(indexName, xyShapeFieldName, multiPolygon);
        assertSearchResponse(searchResponse);
    ...
    public void testIndexPointsIndexedRectangleMatches() throws Exception {
        ...
        createIndex(indexName, Settings.EMPTY, Map.of(xyShapeFieldName, XYShapeFieldMapper.CONTENT_TYPE));
        // Will index two points and search with envelope that will intersect only one point
        queryTestHelper.indexDocumentUsingWKT(indexName, xyShapeFieldName, "POINT(-30 -30)");
        final String secondDocumentID = queryTestHelper.indexDocumentUsingGeoJSON(indexName, xyShapeFieldName, new Point(-45, -50));
