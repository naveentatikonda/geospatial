...
public class XYShapeQueryIT extends AbstractXYShapeQueryTestCase {
    ...
    public void testNullShape() throws Exception {
        queryTestUsingNullShape(indexName, xyShapeFieldName, XYShapeFieldMapper.CONTENT_TYPE);
    ...
    public void testIndexPointsFilterRectangleWithIntersectsRelation() throws Exception {
        createIndex(indexName, Settings.EMPTY, Map.of(xyShapeFieldName, XYShapeFieldMapper.CONTENT_TYPE));
        final String firstDocumentID = indexDocumentUsingWKT(indexName, xyShapeFieldName, "POINT(-30 -30)");
        indexDocumentUsingWKT(indexName, xyShapeFieldName, "POINT(-45 -50)");

        Rectangle rectangle = new Rectangle(-45, 45, 45, -45);
        final SearchResponse searchResponse = searchUsingShapeRelation(indexName, xyShapeFieldName, rectangle, ShapeRelation.INTERSECTS);
        assertSearchResponse(searchResponse);
        assertHitCount(searchResponse, 1);
        MatcherAssert.assertThat(searchResponse.getHits().getAt(0).getId(), equalTo(firstDocumentID));
    ...
    public void testIndexPointsCircle() throws Exception {
        ...
        indexDocumentUsingWKT(indexName, xyShapeFieldName, "POINT(-131 -30)");
        final String secondDocumentID = indexDocumentUsingWKT(indexName, xyShapeFieldName, "POINT(-45 -50)");

        Circle circle = new Circle(-30, -30, 100);

        final SearchResponse searchResponse = searchUsingShapeRelation(indexName, xyShapeFieldName, circle, ShapeRelation.INTERSECTS);
        assertSearchResponse(searchResponse);
        assertHitCount(searchResponse, 1);
    ...
    public void testIndexPointsPolygon() throws Exception {
        ...
        final SearchResponse searchResponse = searchUsingShapeRelation(indexName, xyShapeFieldName, polygon, ShapeRelation.INTERSECTS);
        assertSearchResponse(searchResponse);
        assertHitCount(searchResponse, 1);
        MatcherAssert.assertThat(searchResponse.getHits().getAt(0).getId(), equalTo(firstDocumentID));
    ...
    public void testIndexPointsMultiPolygon() throws Exception {
        ...
        final String firstDocumentID = indexDocumentUsingWKT(indexName, xyShapeFieldName, "POINT(-30 -30)");
        indexDocumentUsingWKT(indexName, xyShapeFieldName, "POINT(-40 -40)");
        final String thirdDocumentId = indexDocumentUsingGeoJSON(indexName, xyShapeFieldName, new Point(-50, -50));
        ...
        final SearchResponse searchResponse = searchUsingShapeRelation(indexName, xyShapeFieldName, multiPolygon, ShapeRelation.INTERSECTS);
        assertSearchResponse(searchResponse);
    ...
    public void testIndexPointsIndexedRectangleMatches() throws Exception {
        ...
        createIndex(indexName, Settings.EMPTY, Map.of(xyShapeFieldName, XYShapeFieldMapper.CONTENT_TYPE));
        // Will index two points and search with envelope that will intersect only one point
        indexDocumentUsingWKT(indexName, xyShapeFieldName, "POINT(-30 -30)");
        final String secondDocumentID = indexDocumentUsingGeoJSON(indexName, xyShapeFieldName, new Point(-45, -50));
