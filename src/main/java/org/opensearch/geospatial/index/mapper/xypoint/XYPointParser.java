                ...
                while (subParser.nextToken() != XContentParser.Token.END_OBJECT) {
                    if (subParser.currentToken() == XContentParser.Token.FIELD_NAME) {
                        ...
                        if ("type".equals(field)) {
                            subParser.nextToken();
                            String geometryType = subParser.text();
                            if ("Point".equals(geometryType) && subParser.nextToken() != XContentParser.Token.END_OBJECT) {
                                if ("coordinates".equals(subParser.currentName())) {
                                    subParser.nextToken();
                                    if (subParser.currentToken() == XContentParser.Token.START_ARRAY) {
                                        // return parseXYPointArray(parser, point, ignoreZValue, x, y);
                                        XYPoint xyPoint = parseXYPointArray(subParser, point, ignoreZValue, x, y);
                                        if (subParser.currentToken() == XContentParser.Token.END_ARRAY) {
                                            subParser.nextToken();
                                        }
                                        return xyPoint;
                                    }
